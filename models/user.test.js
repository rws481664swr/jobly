...
describe("findAll", function () {
    ...
    test("works with job app", async function () {
        ...
        const {rows: [job]} = await db.query(`INSERT INTO jobs (title, salary, equity, company_handle)
                                              VALUES ('engineer', 100000, '0.10', 'c1')
                                              RETURNING *`)
...
describe('apply', () => {
    test('apply for job', async () => {
        const {rows: [user1]} = await db.query(`SELECT *
                                                FROM users
                                                WHERE username = $1`, ['u1'])
        const {rows: [job]} = await db.query(`INSERT INTO jobs (title, salary, equity, company_handle)
                                              VALUES ('engineer', 100000, '0.10', 'c1')
                                              RETURNING *`)
        await User.apply(user1.username, job.id)
